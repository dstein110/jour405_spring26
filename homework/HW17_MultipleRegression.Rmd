...
One variable that appears to be most correlated with higher voter turnout is the percentage of white voters. On the other hand, the percent of unemployed people in a given county has a negative correlation with higher turnout, but in a linear relationship. Other variables like population don't have much of a linear relationship or correlation.  
...
It seems that the multiple variable model predicts turnout pretty well, as all of the coefficients are somewhat large, but not too large or too small. A few of the variables do seem insignificant, such as "percent white" which has a very big range and when compared against turnout, its r-squared value is nearly 70%. You could also draw a similar conclusion about the "percent unemployed" variable, which also has a high r-squared value.
...
Of the three models I tested, none of them are significantly better than the others. One of the models had a significantly worse adjusted r-squared value, but the "best" model (model 2) still had a lower r-squared than the original model. 
...
# Model 3
model3 <- lm(turnout ~ population + pct_bachelors_higher + median_age, data = md_turnout)
summary(model3)

# Model 4
model4 <- lm(turnout ~ pct_white + pct_unemployed + income, data = md_turnout)
summary(model4)
...
You could use this analysis to predict the turnout of a particular election and write a story about it if the model tells you turnout is potentially going to be lower or higher than usual. This analysis also could be used as a potential roadmap for other stories. For example, if the model tells you that income is the most significant factor in predicting voter turnout, you could ask high-income voters about their voting intentions. You could also ask low-income citizens why they do not plan to vote. The model itself may not always be the story, but it can help find other potential leads.
