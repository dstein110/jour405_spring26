...
I think this model is somewhat useful, but definitely flawed. While the errors are evenly dispersed, there is still a wider range in the data and two of the three coefficients are not statistically significant. However, the regression does have a decently high adjusted r-squared, so the model isn't useless. 
...
```{r}
selected_vars <- teams |>  select(`Yards Gained`, Takeaways, `Giveaways`, Wins)
cor(selected_vars)
```
...
From experimenting with the model, it seems points scored clearly has the most impact on wins, as it had by far the largest coefficient. Takeaways appears to have a relatively significant effect on wins, as its responsible for about 28% of the data. Conversely, yards gained does not seem to be impactful on wins at all, as when removing it from the model, the difference in the adjusted r-squared is negligible. 
...
I could use this model to argue what are the most important aspects of the team (offense, defense, etc.). I also think this model could be used to demonstrate the importance of signing defensive players in free agency, as the yards gained category doesn't seem to have an effect on wins. I could mention this model in a data-driven article about how the recent trend of paying offensive positions big money doesn't always necessarily lead to more wins. 
